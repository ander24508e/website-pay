            ...
            <div class="w-full lg:w-1/3 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm p-4 sm:p-6 border border-red-100">
                    ...
                    <form action="{{ route('admin.products.destroy', $product) }}" method="POST"
                        ...
                        @method('DELETE')
                        <button
                            class="w-full bg-red-50 text-red-600 py-2.5 rounded-lg hover:bg-red-100 transition font-medium text-sm border border-red-200">
                            🗑 Eliminar Producto
                        </button>
                    </form>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm p-4 sm:p-6">
                    ...
                    <form id="product-form" action="{{ route('admin.products.update', $product) }}" method="POST"
                        ...
                        <div class="mb-5">
                            ...
                            <p class="text-xs text-gray-500 mb-3">Mantiene precio por litro, galon, caneca u otras
                                presentaciones.</p>
...
@push('scripts')
    @php
        $productVariantsForJs = $product->variants
            ->map(function ($v) {
                return [
                    'id' => $v->id,
                    'name' => $v->name,
    ...
    <script>
        ...
        (function initVariants() {
            ...
            const dbVariants = @json($productVariantsForJs);

            const source = Array.isArray(oldVariants) && oldVariants.length ? oldVariants : dbVariants;

            function renderRow(index, data = {}) {
                const row = document.createElement('div');

                row.className =
                    'grid grid-cols-1 md:grid-cols-12 gap-2 border border-gray-200 rounded-lg p-3 bg-gray-50';
                ...
                row.innerHTML = `
                    ...
                    <input type="text"
                        ...
                        value="${data.name ?? ''}"
                        placeholder="Nombre (Ej: Aceite 20W50)"
                        class="md:col-span-3 border border-gray-200 rounded px-3 py-2 text-sm" />

                    <input type="text"
                        name="variants[${index}][presentation]"
                        value="${data.presentation ?? ''}"
                        placeholder="Presentación (Litro, Galón)"
                        class="md:col-span-2 border border-gray-200 rounded px-3 py-2 text-sm" />

                    <input type="text"
                        name="variants[${index}][specification]"
                        value="${data.specification ?? ''}"
                        placeholder="Especificación (1L, 5L)"
                        class="md:col-span-2 border border-gray-200 rounded px-3 py-2 text-sm" />
                    ...
                    <div class="md:col-span-1 flex items-center justify-between gap-2">
                        ...
                        <button type="button"
                            class="remove-variant text-red-600 text-xs font-semibold">
                            Quitar
                        </button>
                    </div>
                `;
            ...
            if (source.length) {
                source.forEach((variant) => addRow(variant));
            } else {
                addRow({
                    name: '',
                    presentation: 'Unidad',
                    specification: '',
                    price: @json((string) old('price', $product->price)),
                    stock: '',
                    active: true
                });
            }
