...
<div class="container mx-auto px-4 sm:px-6">
    ...
    <div class="flex flex-wrap items-center gap-3 mb-6">
        ...
        <div>
            <h2 class="text-xl sm:text-2xl font-bold text-gray-800">✏️ Editar Servicio</h2>
            <p class="text-gray-400 text-sm">Modificando <strong class="text-gray-600">{{ $service->name }}</strong></p>
        </div>
    </div>
        ...
        <div class="w-full lg:w-1/3 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm p-4 sm:p-6">
                ...
                <div class="flex flex-col items-center mb-4">
                    ...
                    <p class="text-xs text-gray-400 text-center" id="img-name">
                        {{ $service->image ? basename($service->image) : 'Sin imagen' }}
                    </p>
                </div>

                <input type="file" name="image" id="image-input" accept="image/*"
                       form="service-form" class="hidden" onchange="previewImage(this)">

                <button type="button" onclick="document.getElementById('image-input').click()"
                        class="w-full bg-gray-900 text-white py-2.5 rounded-lg hover:bg-gray-700 transition font-medium text-sm">
                    ☁️ {{ $service->image ? 'Cambiar Imagen' : 'Subir Imagen' }}
                </button>
            ...
            <div class="bg-white rounded-xl shadow-sm p-4 sm:p-6">
                ...
                <label class="flex items-center gap-3 cursor-pointer">
                    ...
                    <div>
                        <p class="text-sm font-semibold text-gray-700">Servicio activo</p>
                ...
                <form id="service-form" action="{{ route('admin.services.update', $service) }}" method="POST"
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" rows="4"
                                  class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-300 bg-gray-50 resize-none">{{ old('description', $service->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
