...
<div class="container mx-auto px-4 sm:px-6 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            ...
            <p class="text-2xl font-bold text-gray-800 mt-2">${{ number_format((float) ($variant->cost_price ?? 0), 2) }}</p>
        </div>
    </div>
    ...
    <form method="GET" class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <tbody class="divide-y">
                @forelse($movements as $movement)
                    <tr class="{{ $movement->voided_at ? 'text-gray-400 line-through' : '' }}">
                        ...
                        <td class="px-3 py-3 text-center">${{ number_format((float) ($movement->unit_cost ?? 0), 2) }}</td>
                        <td class="px-3 py-3 text-center">${{ number_format((float) ($movement->total_cost ?? 0), 2) }}</td>
                        <td class="px-3 py-3 text-center">{{ $movement->balance_quantity ?? $movement->stock_after ?? '-' }}</td>
                        <td class="px-3 py-3 text-center">${{ number_format((float) ($movement->balance_unit_cost ?? 0), 2) }}</td>
                        <td class="px-3 py-3 text-center">${{ number_format((float) ($movement->balance_total_cost ?? 0), 2) }}</td>
                        <td class="px-3 py-3 text-center">{{ $movement->user?->name ?? 'Sistema' }}</td>
                    </tr>
                @empty
                    <tr><td colspan="12" class="px-4 py-8 text-center text-gray-400">Sin movimientos para este producto.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
