...
@section('title', 'Editar Precio por Vehiculo')
...
<div class="mx-auto w-full max-w-full overflow-x-hidden px-3 pb-4 sm:px-6">
    <div class="mb-6 flex flex-wrap items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-bold text-gray-800 sm:text-2xl">Editar Precio por Vehiculo</h2>
            <p class="text-sm text-gray-400">Modificando precio de <strong class="text-gray-600">{{ $service->name }}</strong></p>
        </div>
    </div>

    <div class="mx-auto w-full max-w-6xl">
        <div class="rounded-xl bg-white p-4 shadow-sm sm:p-8">
            <form action="{{ route('admin.catalog-service-prices.update', $serviceVehicleTypePrice) }}" method="POST">
                @csrf
                @method('PUT')
