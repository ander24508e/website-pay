        ...
        <div class="grid grid-cols-1 lg:grid-cols-[320px_minmax(0,1fr)] gap-4">
            ...
            <section class="min-w-0 flex flex-col gap-4">
                <div class="bg-white rounded-xl shadow-sm overflow-hidden flex flex-col">
                    <div
                        class="p-4 sm:p-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shrink-0">
                        <div class="min-w-0">
                    ...
                    @if ($catalogType->categories->isEmpty())
                        <div
                            class="m-4 sm:m-5 rounded-lg border border-dashed border-gray-200 bg-gray-50 p-5 text-sm text-gray-500">
                                    ...
                                    <div class="flex items-start justify-between gap-3">
                                        <div class="min-w-0">
                                            ...
                                            <p class="text-xs text-gray-400 mt-1 break-words">
                                                {{ $category->slug ?: 'Sin slug' }}</p>
                                        </div>
                                        @if ($category->active)
                                            <span
                                                class="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-[11px] font-medium shrink-0">Activo</span>
                                        @else
                                            <span
                                                class="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full text-[11px] font-medium shrink-0">Oculto</span>
                                        @endif
                                    </div>
                                    <p class="text-sm text-gray-500 mt-3 break-words">
                                        {{ $category->description ?: 'Sin descripción' }}</p>
                        ...
                        <div class="hidden md:block overflow-y-auto overflow-x-hidden max-h-[320px] flex-1">
                            <table class="w-full table-fixed text-sm text-left">
                                <thead class="bg-gray-50 border-b sticky top-0 z-10">
                                    <tr>
                                        <th class="w-[28%] px-3 py-2.5 text-gray-600 font-semibold text-center">Categoría
                                        </th>
                                        <th
                                            class="w-[18%] px-3 py-2.5 text-gray-600 font-semibold text-center hidden sm:table-cell">
                                            Slug</th>
                                        <th class="w-[16%] px-3 py-2.5 text-gray-600 font-semibold text-center">
                                            {{ $itemShortLabel }}</th>
                                        <th class="w-[12%] px-3 py-2.5 text-gray-600 font-semibold text-center">Estado</th>
                                        <th class="w-[24%] px-3 py-2.5 text-gray-600 font-semibold text-center">Acciones
                                        </th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y">
                                    @foreach ($catalogType->categories as $category)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-3 py-2.5 text-gray-600 text-center truncate hidden sm:table-cell">
                                                {{ $category->slug ?: '-' }}</td>
                                            <td class="px-3 py-2.5 text-gray-700 text-center">{{ $category->items_count }}
                                            </td>
                                            <td class="px-3 py-2.5 text-center">
                                                @if ($category->active)
                                                    <span
                                                        class="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-[11px] font-medium">Activo</span>
                                                @else
                                                    <span
                                                        class="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full text-[11px] font-medium">Oculto</span>
                                                @endif
                                            </td>
                                            <td class="px-3 py-2.5 text-center">
                                                <div class="flex items-center justify-center gap-1">
                                                    <a href="{{ route('admin.catalog-items.index', ['catalog_type_id' => $category->catalog_type_id, 'catalog_category_id' => $category->id]) }}"
                                                        ...
                                                        title="Ver {{ $itemPlural }}"
                                                        aria-label="Ver {{ $itemPlural }}">
                                                        <x-heroicon-o-eye class="w-4 h-4" />
                                                    </a>
                                                    <a href="{{ route('admin.catalog-categories.edit', ['catalogCategory' => $category, 'return_to_type' => 1]) }}"
                ...
                <div class="bg-white rounded-xl shadow-sm overflow-hidden flex flex-col">
                    <div
                        class="p-4 sm:p-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shrink-0">
                        <div class="min-w-0">
                            <h3 class="text-lg font-bold text-gray-800">{{ $itemPluralTitle }} dentro de
                                {{ $catalogType->name }}</h3>
                    ...
                    @else
                        ...
                        <div class="hidden md:block overflow-y-auto overflow-x-hidden max-h-[320px] flex-1">
                            <table class="w-full table-fixed text-sm text-left">
                                <thead class="bg-gray-50 border-b sticky top-0 z-10">
                                    <tr>
                                        <th class="w-[28%] px-3 py-2.5 text-gray-600 font-semibold text-center">
                                            {{ $itemSingularTitle }}</th>
                                        <th
                                            class="w-[18%] px-3 py-2.5 text-gray-600 font-semibold text-center hidden sm:table-cell">
                                            Categoría</th>
                                        <th class="w-[13%] px-3 py-2.5 text-gray-600 font-semibold text-center">Precio</th>
                                        <th
                                            class="w-[10%] px-3 py-2.5 text-gray-600 font-semibold text-center hidden md:table-cell">
                                            Pres.</th>
                                        <th class="w-[11%] px-3 py-2.5 text-gray-600 font-semibold text-center">Estado</th>
                                        <th class="w-[20%] px-3 py-2.5 text-gray-600 font-semibold text-center">Acciones
                                        </th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y">
                                    @foreach ($catalogType->items as $item)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td
                                                class="px-3 py-2.5 text-gray-600 text-center truncate hidden sm:table-cell">
                                                {{ $item->category?->name ?: 'Sin categoría' }}</td>
                                            <td
                                                class="px-3 py-2.5 font-semibold text-gray-800 text-center whitespace-nowrap">
                                                ${{ number_format($item->display_price, 2) }}
                                            </td>
                                            <td class="px-3 py-2.5 text-gray-700 text-center hidden md:table-cell">
                                            ...
                                            <td class="px-3 py-2.5 text-center">
                                                <div class="flex items-center justify-center gap-1">
                                                    <a href="{{ route('admin.catalog-items.show', ['catalogItem' => $item, 'return_to_type' => 1]) }}"
                                                        class="inline-flex items-center justify-center w-7 h-7 rounded-lg text-blue-600 hover:text-blue-800 hover:bg-blue-50 transition"
                                                        title="Ver {{ $itemSingular }}"
                                                        aria-label="Ver {{ $itemSingular }}">
                                                        <x-heroicon-o-eye class="w-4 h-4" />
                                                    </a>
                                                    <a href="{{ route('admin.catalog-items.edit', ['catalogItem' => $item, 'return_to_type' => 1]) }}"
                                                        class="inline-flex items-center justify-center w-7 h-7 rounded-lg text-yellow-600 hover:text-yellow-800 hover:bg-yellow-50 transition"
                                                        title="Editar {{ $itemSingular }}"
                                                        aria-label="Editar {{ $itemSingular }}">
                                                        <x-heroicon-o-pencil-square class="w-4 h-4" />
                                                    </a>
                                                    <a href="{{ route('admin.catalog-service-prices.create', ['catalogItem' => $item->id, 'catalog_type_id' => $catalogType->id, 'return_to_type' => 1]) }}"
