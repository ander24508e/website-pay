...
<div class="container mx-auto px-4 sm:px-6">
    <div class="flex flex-wrap items-center gap-2 text-xs text-gray-400 uppercase tracking-wide mb-4">
        ...
        <span>/</span>
        <a href="{{ route('admin.catalog-types.index') }}" class="hover:text-gray-600 transition">Subnegocios</a>
        <span>/</span>
    ...
    <div class="flex flex-wrap items-center gap-3 mb-6">
        ...
        <div>
            ...
            <p class="text-gray-400 text-sm">Administra este catalogo y los items que se mostraran en tu website.</p>
        </div>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
            <p class="text-xs uppercase tracking-wide text-gray-400 font-semibold">Subnegocio</p>
            <p class="text-lg font-bold text-gray-800 mt-2">{{ $catalogType->name }}</p>
            <p class="text-xs text-gray-400 mt-1">Eje principal del catalogo publico</p>
        </div>
        <a href="{{ route('admin.catalog-categories.index', ['catalog_type_id' => $catalogType->id]) }}" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:border-gray-300 transition">
            ...
            <p class="text-xs text-gray-400 mt-1">Ver categorias de este catalogo</p>
        </a>
        <a href="{{ route('admin.catalog-items.index', ['catalog_type_id' => $catalogType->id]) }}" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:border-gray-300 transition">
            ...
            <p class="text-xs text-gray-400 mt-1">Ver items de este catalogo</p>
        </a>
    </div>
        ...
        <div class="xl:col-span-2 space-y-6">
            <div class="bg-white rounded-xl shadow-sm p-4 sm:p-6">
                <div class="flex items-center justify-between gap-3 mb-4">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">Categorias de este catalogo</h3>
                        <p class="text-sm text-gray-400">Te ayudan a organizar lo que se vera dentro del subnegocio.</p>
                    </div>
                    <a href="{{ route('admin.catalog-categories.create', ['catalog_type_id' => $catalogType->id, 'return_to_type' => 1]) }}"
                ...
                @if($catalogType->categories->isEmpty())
                    <div class="rounded-lg border border-dashed border-gray-200 bg-gray-50 p-5 text-sm text-gray-500">
                        Este catalogo aun no tiene categorias. Puedes crear items sin categoria o agregar una mas adelante para ordenar mejor.
                    </div>
                @else
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                <div class="p-4 sm:p-6 border-b border-gray-100 flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">Items dentro de {{ $catalogType->name }}</h3>
                        <p class="text-sm text-gray-400">Estos son los items que podran mostrarse en el website dentro de este catalogo.</p>
                    </div>
                    <a href="{{ route('admin.catalog-items.create', ['catalog_type_id' => $catalogType->id, 'return_to_type' => 1]) }}"
                ...
                @if($catalogType->items->isEmpty())
                    <div class="p-6 sm:p-8 text-center text-gray-500">
                        <p class="font-medium text-gray-700 mb-2">Todavia no hay items en este catalogo.</p>
                        <p class="text-sm mb-5">Empieza creando el primero para que luego se vea en el catalogo del website.</p>
                        <a href="{{ route('admin.catalog-items.create', ['catalog_type_id' => $catalogType->id, 'return_to_type' => 1]) }}"
