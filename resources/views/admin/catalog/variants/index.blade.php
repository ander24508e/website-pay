...
@section('title', 'Variantes Universales')
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        <div class="min-w-0">
            <div class="flex items-center gap-2">
                ...
                <div>
                    <h2 class="text-xl sm:text-2xl font-bold text-gray-800">Variantes Universales</h2>
                    <p class="text-gray-500 text-sm mt-1">Presentaciones, tamaños y versiones para los items del nuevo catalogo.</p>
                </div>
            </div>
        </div>
        ...
        <form method="GET" action="{{ route('admin.catalog-variants.index') }}" class="flex-1 max-w-xl">
            <div class="relative">
                ...
                <input type="search" name="q" value="{{ request('q') }}" placeholder="Buscar por variante, SKU, item o tipo..." class="w-full rounded-lg border border-gray-200 bg-white py-2.5 pl-10 pr-4 text-sm text-gray-700 placeholder:text-gray-400 focus:border-gray-400 focus:outline-none focus:ring-0">
            </div>
        </form>
        ...
        <div class="overflow-x-auto overflow-y-auto max-h-[70vh]">
            <table class="min-w-[1100px] w-full text-sm text-left">
                <thead class="bg-gray-50 border-b sticky top-0 z-10">
                    <tr>
                        <th class="px-4 sm:px-6 py-3 sm:py-4">Variante</th>
                ...
                <tbody class="divide-y">
                    @forelse($variants as $variant)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 sm:px-6 py-3 sm:py-4 text-gray-700">
                                {{ $variant->item->type->name ?? 'Sin tipo' }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-10 text-gray-400">No hay variantes universales registradas</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
