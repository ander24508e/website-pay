    ...
    <div class="flex min-h-screen">
        <aside class="w-64 bg-gray-900 text-white flex flex-col fixed h-full">
            <div class="p-6 border-b border-gray-700">
                ...
                <p class="text-gray-400 text-sm mt-1">{{ auth()->user()?->name ?? 'Admin' }}</p>
            </div>
            ...
            <nav class="flex-1 p-4 space-y-1">
                ...
                <a href="{{ route('admin.empresa.edit') }}"
                    class="flex items-center gap-3 px-4 py-2.5 rounded-lg hover:bg-gray-700 transition {{ request()->routeIs('admin.empresa.*') ? 'bg-gray-700' : '' }}">
                    🏢 Mi Empresa
                </a>
