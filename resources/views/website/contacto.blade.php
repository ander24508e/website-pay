    ...
    <div class="contact-layout fade-up">
        <aside class="contact-info-col">
                ...
                <div class="contact-row">
                    ...
                    <div>
                        ...
                        <a href="{{ $waUrl }}" target="_blank"
                            rel="noopener noreferrer">{{ $empresa->telefono_contacto }}</a>
            ...
            <div class="map-wrap">
                ...
                <div class="map-frame">
                    <iframe src="{{ $empresa->ubicacion_mapa_url }}" allowfullscreen loading="lazy"
        ...
        <section class="contact-form-card">
            ...
            <form id="contact-booking-form" class="contact-form-grid" novalidate>
                ...
                <div id="contact-vehicle-wrap" hidden>
                    <label for="contact-vehicle">Servicios </label>
        ...
        (() => {
            ...
            function getVehiclePriceEntry(item, vehicleTypeId, vehicleSpecificationId = null) {
                ...
                if (vehicleSpecificationId) {
                    const exactPrice = prices.find((price) => Number(price.vehicle_specification_id) === Number(
                        vehicleSpecificationId));
                    if (exactPrice) return exactPrice;
                }

                return prices.find((price) => !price.vehicle_specification_id && Number(price.vehicle_type_id) ===
                    Number(vehicleTypeId)) || null;
            }
            ...
            function updateVehicleHint() {
                ...
                const duration = option.dataset.durationMinutes ?
                    ` Duración aproximada: ${option.dataset.durationMinutes} min.` : '';
            ...
            function renderVehicleOptions(item) {
                ...
                if (compatibleVehicles.length) {
                    ...
                    compatibleVehicles.forEach((vehicle) => {
                        const priceEntry = getVehiclePriceEntry(item, vehicle.vehicle_type_id, vehicle
                ...
                if (compatibleSpecifications.length) {
                    ...
                    compatibleSpecifications.forEach((specification) => {
                        const priceEntry = getVehiclePriceEntry(item, specification.vehicle_type_id,
                            specification.id);
                        const option = document.createElement('option');
                ...
                if (directPrices.length) {
                    ...
                    directPrices.forEach((priceEntry) => {
                        ...
                        const option = document.createElement('option');
                        const price = setVehicleOptionPriceData(option, priceEntry, basePrice);
                        option.value = vehicleSpecificationId ? `spec:${vehicleSpecificationId}` :
                            `type:${vehicleTypeId}`;
                        if (vehicleSpecificationId) option.dataset.vehicleSpecificationId = String(
                            vehicleSpecificationId);
                        option.dataset.vehicleTypeId = String(vehicleTypeId);
                        option.textContent =
                            `${priceEntry.vehicle_name || priceEntry.vehicle_type_name || 'Vehículo'} - ${formatCatalogPrice(price)}`;
                        group.appendChild(option);
                    });
            ...
            btn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (!response.ok) {
                        showError((data && data.message ? data.message : 'No se pudo crear la reserva.') +
